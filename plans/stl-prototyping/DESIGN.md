...
| Mesh carries a coordinate that is not a real number | `BAD_COORDINATE` **error** naming the triangle; the mesh still loads, is still previewable, and every measurement that does not touch that triangle is still reported | *(User decision, 2026-08-28.)* A recipe that divides by zero or overflows writes `Infinity` or `NaN` into a vertex, and one such vertex turns the bounding box into `Infinity mm` and the volume, normals and centroid into blanks — measured, T03. Report-never-block applies: the user still gets the 99% of the shape that is fine, and an error that names the triangle is what sends them to the right line of the recipe. |
| Mesh carries a coordinate that is a real number but absurdly large | `COORDINATE_OUT_OF_RANGE` **error** naming the triangle, exactly as `BAD_COORDINATE` does, with its own code and its own wording: the part is so large that the measurements below it can no longer be trusted. The mesh still loads and still previews. | *(User decision, 2026-08-28, choosing this over folding it into `BAD_COORDINATE` and over a ten-metre threshold.)* It is a **different fix from a different mistake** — a runaway scale in a recipe, not a division by zero — and a message saying "not a real number" about a real number sends the user looking for the wrong thing. The threshold is not invented: above `weldTolerance * 2^53`, about 900 000 km at the default tolerance, `core/solidity.js` can no longer tell two vertices apart at all, and that line sits far below the ~1.3e19 mm where a triangle's area stops fitting in float32 — the hole T03 measured and left open. A ten-metre threshold was rejected because a ten-metre part is already reported by `DOES_NOT_FIT`. |
